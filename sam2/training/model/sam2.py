...
import logging
import sys, os
sys.path.append("/media/voanhnhat/SDD_OUTSIDE1/PROJECT_DETECT_OBJECT")
import numpy as np
import torch
...
from sam2.sam2.modeling.sam2_utils import (
    get_1d_sine_pe,
    get_next_point,
    sample_box_points,
    select_closest_cond_frames,
)
from sam2.training.utils.misc import concat_points
...
class SAM2Train(SAM2Base):
    ...
    def forward(self, input: BatchedVideoDatapoint):
        if self.training or not self.forward_backbone_per_frame_for_eval:
            ...
            flat_imgs = getattr(input, "flat_img_batch", None)
            if flat_imgs is None and hasattr(input, "extra"):
                flat_imgs = input.extra.get("flat_img_batch", input.img_batch)
            backbone_out = self.forward_image(flat_imgs)
    ...
    def _iter_correct_pt_sampling(
        ...
        for _ in range(self.num_correction_pt_per_frame):
            ...
            if isinstance(point_inputs, dict):
                if not isinstance(point_inputs.get("point_coords", None), torch.Tensor):
                    point_inputs["point_coords"] = torch.tensor([], dtype=new_points.dtype, device=new_points.device).view(1, 0, 2)
                if not isinstance(point_inputs.get("point_labels", None), torch.Tensor):
                    point_inputs["point_labels"] = torch.tensor([], dtype=new_labels.dtype, device=new_labels.device).view(1, 0)
                point_inputs = concat_points(point_inputs, new_points, new_labels)
            else:
                # fallback nếu lỡ có string hoặc None
