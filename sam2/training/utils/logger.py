...
class TensorBoardWriterWrapper:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        If distributed training is not initialized, fallback to single-GPU mode.
        """
        ...
        try:
            ...
            if isinstance(ranks, (list, tuple)):
                if len(ranks) == 3:
                    self._local_rank, self._rank, self._world_size = ranks
                elif len(ranks) == 2:
                    self._local_rank, self._rank = ranks
                    self._world_size = 1
                else:
                    self._local_rank, self._rank, self._world_size = 0, 0, 1
            else:
                self._local_rank, self._rank, self._world_size = 0, int(ranks), 1
        except Exception as e:
            logging.warning(f"[Logger] Failed to read ranks safely: {e}")
            self._local_rank, self._rank, self._world_size = 0, 0, 1
        ...
        if self._rank == 0:
            logging.info(
                f"TensorBoard SummaryWriter instantiated. "
                f"Files will be stored in: {path} (rank={self._rank}, world_size={self._world_size})"
            )
            ...
            logging.debug(
                f"[TensorBoardLogger] Skipping log on this rank ({self._rank})."
            )
