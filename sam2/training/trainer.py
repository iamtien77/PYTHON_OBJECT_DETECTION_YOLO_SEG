...
SAM2_SUB = os.path.join(SAM2_ROOT, "sam2")

for p in (PROJECT_ROOT, SAM2_ROOT, SAM2_SUB):
    if p not in sys.path:
        sys.path.insert(0, p)

print("[DEBUG] Added to sys.path:", PROJECT_ROOT, SAM2_ROOT, SAM2_SUB)
...
class Trainer:
    ...
    def _setup_device(self, accelerator):
        ranks = get_machine_local_and_dist_rank()
        if isinstance(ranks, (list, tuple)):
            if len(ranks) == 3:
                self.local_rank, self.distributed_rank, _ = ranks
            elif len(ranks) == 2:
                self.local_rank, self.distributed_rank = ranks
            else:
                raise ValueError(f"Unexpected rank tuple length: {len(ranks)}")
        else:
            self.local_rank = 0
        ...
        if accelerator == "cuda":
            if not torch.cuda.is_available():
                raise RuntimeError("CUDA not available but accelerator='cuda'.")
        ...
        import logging
        logging.info(f"[SETUP_DEVICE] Device={self.device}, Local Rank={self.local_rank}, Distributed Rank={self.distributed_rank}")
    ...
    def _setup_ddp_distributed_training(self, distributed_conf, accelerator):
        ...
        try:
            ...
            print(f"[INFO] DDP initialized successfully on rank {self.distributed_rank}")
        except Exception as e:
            print(f"[ERROR] Failed to init DDP on rank {self.distributed_rank}: {e}")
            raise
    ...
    def _step(
        ...
        for key, loss_fn in self.loss.items():
            if key not in outputs:
                print(f"[WARN] Skipping loss for missing key: {key}")
                continue
            valid_losses.append(loss_fn(outputs[key], targets))

        if not valid_losses:
            print("[ERROR] No valid losses computed (no matching keys in outputs).")
            return {CORE_LOSS_KEY: torch.tensor(0.0, device=self.device)}, len(targets), {}
    ...
    def val_epoch(self, val_loader, phase):
        ...
        for data_iter, batch in enumerate(val_loader):
            ...
            with torch.no_grad():
                with torch.amp.autocast('cuda', enabled=(self.optim_conf.amp.enabled if self.optim_conf else False), dtype=get_amp_type(self.optim_conf.amp.amp_dtype) if self.optim_conf else None):
    ...
    def _run_step(
        ...
        with torch.amp.autocast('cuda', enabled=self.optim_conf.amp.enabled, dtype=get_amp_type(self.optim_conf.amp.amp_dtype)):
            loss_dict, batch_size, extra_losses = self._step(
                batch,
                self.model,
                phase,
    ...
    def _setup_components(self):
        ...
        if isinstance(self.model_conf, (dict, OmegaConf)):
            self.model = instantiate(self.model_conf, _convert_="all")
        else:
            self.model = self.model_conf
