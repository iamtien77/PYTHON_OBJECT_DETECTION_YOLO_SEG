...
class CXBlock(nn.Module):
    ...
    def __init__(self, dim, kernel_size=7, padding=3, drop_path=0.0, layer_scale_init_value=1e-6, use_dwconv=True):
        ...
        if isinstance(layer_scale_init_value, str):
            layer_scale_init_value = float(layer_scale_init_value) if layer_scale_init_value.replace('.', '', 1).isdigit() else 1e-6
        if isinstance(drop_path, str):
            drop_path = float(drop_path) if drop_path.replace('.', '', 1).isdigit() else 0.0
